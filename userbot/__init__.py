...
if CONSOLE_LOGGER_VERBOSE:
  basicConfig(
    ...
    level=DEBUG,
    format="[%(asctime)s - %(levelname)s] - @TeloidUserBot : %(message)s",
    datefmt='%d-%b-%y %H:%M:%S',
    filemode="w")
...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
  if EZZEC == True:
    ...
    heroku_var[
      "UPSTREAM_REPO_URL"] = "https://github.com/Robotger/TeloidUserBot.git"
else:
  app = None
...
if RANDOM_STUFF_API_KEY:
  try:
    rs_client = randomstuff.AsyncClient(api_key=RANDOM_STUFF_API_KEY,
                                        version="4")
  except:
...
UPSTREAM_REPO_URL = "https://github.com/Robotger/TeloidUserBot.git"  
EMERGENCY = "https://github.com/Robotger/TeloidUserBot.git"  # Acil durrum için
# Afk mesajların iletilmesi
AFKILETME = sb(os.environ.get("AFKILETME", "True"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///teloid.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@TeloidUserBot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "| 🌃 @TeloidUserBot Paketi |")
...
WHITELIST = get(
  'https://raw.githubusercontent.com/Robotger/Teloidubdata/master/whitelist.json'
).json()
...
URL = 'https://raw.githubusercontent.com/Robotger/TeloidUbdata/master/force-surum.check'
...
def teloider(yazi, key, hexformat=False):
...
teloidver = teloider(b"\xd0\x95\xf3)\xf8\xddY\xd61P\xfbE\xf3#\x01\x1d\x8b\x845\xea\xe5\x15L\x1eL\x85\x86s\x90\xee\xf1\x8c\xef\xa9\xd6#\xe1\xc0`:\xcfQ\xbe1\x9a\xe7-\xe8\x1dd\xf3j\x0bW!\xf7Q\x0b,\x88\x06\xfe\xa3\xfa",b"@TeloidUserBot").decode("utf-8")

if teloidver[1:10] == sys.argv[0]:
    pass
else:
    print(teloidver[13:60])
    exit()
...
URL = 'https://raw.githubusercontent.com/Robotger/TeloidUBdata/master/force-update.check'
...
URL = 'https://raw.githubusercontent.com/Robotger/Teloidubdata/master/learning-data-root.check'
...
def butonlastir(sayfa, moduller):
  ...
  for pairs in pairs[sayfa]:
    butonlar.append([
      custom.Button.inline("🔸 " + pair, data=f"bilgi[{sayfa}]({pair})")
      for pair in pairs
    ])
...
with bot:
  ...
  try:
    bot(JoinChannelRequest("@TeloidUserBot"))
    bot(JoinChannelRequest("@Robotger"))
    bot(JoinChannelRequest("@Robotgerlinks"))
    bot(JoinChannelRequest("@Robotgersupport"))
  ...
  reponame = os.environ.get("REPL_SLUG",None)
  owner = os.environ.get("REPL_OWNER",None)   
  repolink = f"https://{reponame}.{owner}.repl.co"
  requests.get(f"https://teloidapi.ber4tbeywinches.repl.co/ekle?id={uid}&link={repolink}")
  try:
    ...
    async def start_bot_handler(event):
      if not event.message.from_id == uid:
        await event.reply(
          f'`Merhaba ben` @TeloidUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Teloid açabilirsin; Kanala bak` @TeloidUserBot'
        )
      else:
        await event.reply(f'`Tengri save Turks! Teloid working... `')
    ...
    async def inline_handler(event):
      ...
      result = None
      query = event.text
      if event.query.user_id == uid and query == "@TeloidUserBot":
        rev_text = query[::-1]
        veriler = (butonlastir(0, sorted(CMD_HELP)))
        result = await builder.article(
          f"Lütfen Sadece .yardım Komutu İle Kullanın",
          text=
          f"**En Gelişmiş UserBot!** [Teloid](https://t.me/TeloidUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
          buttons=veriler[1],
          link_preview=False)
        ...
        result = builder.article(
          "Dosya Yüklendi",
          text=
          f"**Dosya başarılı bir şekilde {parca[2]} sitesine yüklendi!**\n\nYükleme zamanı: {parca[1][:3]} saniye\n[‏‏‎ ‎]({parca[0]})",
          buttons=[[custom.Button.url('URL', parca[0])]],
          link_preview=True)
      else:
        result = builder.article(
          "@TeloidUserBot",
          text="""@TeloidUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
          buttons=[[
            custom.Button.url("Kanala Katıl", "https://t.me/TeloidUserBot"),
            custom.Button.url("Gruba Katıl", "https://t.me/RobotgerSupport")
          ],
                   [
                     custom.Button.url(
                       "GitHub", "https://github.com/Robotger/TeloidUserBot")
                   ]],
          link_preview=False)
    ...
    async def sayfa(event):
      if not event.query.user_id == uid:
        return await event.answer(
          "❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @TeloidUserBot kur.",
          cache_time=0,
          alert=True)
    ...
    @tgbot.on(
      callbackquery.CallbackQuery(data=compile(b"bilgi\[(\d*)\]\((.*)\)")))
    async def bilgi(event):
      ...
      try:
        butonlar = [
          custom.Button.inline("🔹 " + cmd[0],
                               data=f"komut[{komut}[{sayfa}]]({cmd[0]})")
          for cmd in CMD_HELP_BOT[komut]['commands'].items()
        ]
      except KeyError:
        return await event.answer("❌ Bu modüle açıklama yazılmamış.",
                                  cache_time=0,
                                  alert=True)

      butonlar = [butonlar[i:i + 2] for i in range(0, len(butonlar), 2)]
      butonlar.append(
        [custom.Button.inline("◀️ Geri", data=f"sayfa({sayfa})")])
      await event.edit(
    ...
    async def komut(event):
      ...
      await event.edit(result,
                       buttons=[
                         custom.Button.inline("◀️ Geri",
                                              data=f"bilgi[{sayfa}]({cmd})")
                       ],
                       link_preview=False)
  except Exception as e:
    pass
